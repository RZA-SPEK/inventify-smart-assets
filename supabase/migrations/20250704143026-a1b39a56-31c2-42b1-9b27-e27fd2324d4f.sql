...
-- Fix the database role issue that's causing the "admin" role error
-- The issue is that some policies or functions are referencing a PostgreSQL role that doesn't exist

-- First, let's ensure the get_current_user_role function is properly defined
...
-- Drop and recreate all problematic RLS policies
-- Assets policies
...
-- Reservations policies - add missing SELECT policy
...
-- Fix any other functions that might reference non-existent roles
